...
pub enum TokenKind {
    ...
    /// than one the program calls by name. See [`crate::syntax::ast::Op`].
    Op,
    Class,
    Extends,
    ...
    /// [`crate::syntax::ast::Openness`] for which door each one closes — [`Self::Final`]
    ...
    Private,
    Protected,
    /// The top type, as an annotation. `_` means the same and is not a keyword —
    ...
    ///
    /// One character on its own. v0.7 tranche 5 adds `??` and `?.`, which are
...
impl TokenKind {
    ...
    ///
    /// Here rather than in the language server, which is where it used to live as ten
...
/// [`crate::syntax::doc`]'s, and it needs the declaration in hand to do it.
#[derive(Clone, Debug, PartialEq)]
pub struct DocBlock {
...
mod tests {
    ...
    fn every_keyword_explains_itself() {
        // The guard the ten-entry table in the language server could not have: it covered
