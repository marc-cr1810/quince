...
pub struct Native {
    ...
    /// receiver at all — an asymmetry `builtins::stdlib` explains and this field is
    ...
    /// they hover over it.
    ///
    /// Beside the implementation rather than in the language server, which is where it
...
pub enum Value {
    ...
    /// can be bound, passed, and stored. The handle is an [`crate::runtime::env::Globals`]
...
impl Value {
    ...
    // the evaluator: a worse message beats that trade every time.
    //
